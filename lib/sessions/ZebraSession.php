<?php
	/// This is an external library MEMS uses.  All SQL queries have been modified to work with MEMS' setup, but the
	///  bulk of the code should be the same.
 ...
 *
 *  This class is was inspired by John Herren's code from the Trick out your session handler article
 *  (http://devzone.zend.com/node/view/id/141) and Chris Shiflett's code from his book Essential PHP Security
 *  (http://phpsecurity.org/code/ch08-2), chapter 8, Shared Hosting, Pg. 78-80.
 *
 ...
 *  @version    1.0.8 (last revision: December 27, 2010)
 *  @copyright  (c) 2006 - 2010 Stefan Gabos
...
class Zebra_Session{
     ...
     *  @return void
     */
    function Zebra_Session($session_lifetime = '', $gc_probability = '', $gc_divisor = '', $security_code = 'sEcUr1tY_c0dE', $table_name = 'sessiondata'){
		...
		if ($gc_probability != '' && is_integer($gc_probability))
			ini_set('session.gc_probability', $gc_probability);	 // set the new value

		// if $gc_divisor is specified and is an integer number
		if ($gc_divisor != '' && is_integer($gc_divisor))
			ini_set('session.gc_divisor', $gc_divisor);	// set the new value
    ...
    function destroy($session_id){
		...
		$deleteSessionQuery = sprintf(
				"DELETE FROM %s
				WHERE SessionID='%s'",
				mysql_real_escape_string($this->table_name),
				mysql_real_escape_string($session_id));
		try{
			$deleteSessionResult = Database::getInstance()->query($deleteSessionQuery, 2, 1);
		}
    ...
    function gc($maxlifetime){
		...
		$deleteSessionsQuery = sprintf(
				"DELETE FROM %s
				WHERE SessionID < '%s'",
				mysql_real_escape_string($this->table_name),
				mysql_real_escape_string(time() - $maxlifetime));
		$deleteSessionsResult = Database::getInstance()->query($deleteSessionsQuery);
    }
    ...
    function get_active_sessions(){
		...
		$countSessionsQuery = sprintf(
				"SELECT COUNT(SessionID) as count
				FROM %s",
				mysql_real_escape_string($this->table_name));
		$countSessionsResult = Database::getInstance()->query($countSessionsQuery, 1, 1);

        // return the number of found rows
        return $countSessionsResult->fetch_object()->count;
    }
    ...
    function read($session_id){
		...
		$userAgent = md5((isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '') . $this->securityCode);
		$readSessionQuery = sprintf(
				"SELECT session_data
				FROM %s
				WHERE SessionID='%s'
					AND session_expire > '%s'
					AND http_user_agent = '%s'
				LIMIT 1",
				mysql_real_escape_string($this->table_name),
				mysql_real_escape_string($session_id),
				time(),
		...
		try{
			$readSessionResult = Database::getInstance()->query($readSessionQuery, 1, 1);
		}
		catch(MySQLException $e){ return ''; }// on error return an empty string - this HAS to be an empty string

		// if anything was found, return found data
		return $readSessionResult->fetch_object()->session_data;
    }
    ...
    function write($session_id, $session_data){
		...
		$userAgent = md5((isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '') . $this->securityCode);
		$insertSessionQuery = sprintf(
				"INSERT INTO %s
					(SessionID, http_user_agent, session_data, session_expire) VALUES
					('%s', '%s', '%s', '%s')
				ON DUPLICATE KEY UPDATE
					session_data='%s', session_expire='%s'",
				mysql_real_escape_string($this->table_name),
				mysql_real_escape_string($session_id),
				mysql_real_escape_string($userAgent),
				...
				mysql_real_escape_string(time() + $this->session_lifetime));
		try{
			$insertSessionResult = Database::getInstance()->query($insertSessionQuery, 2, 1);
		}
		catch(MySQLException $e){ return false; }

		// if the row was updated
		return true;
//		else // if the row was inserted
//			return '';
    }
}
?>
