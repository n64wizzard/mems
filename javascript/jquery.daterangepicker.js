...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			...
			{text: 'This Week',
				...
				dateEnd: function(){
					if(Date.today().is().saturday())
						...
						return Date.today().moveToDayOfWeek(6);
				}
			},
			{text: 'This Month',
			...
			{text: 'Next 365 Days',
				dateStart: function(){return Date.today().setTimezoneOffset(0);},
				dateEnd: function(){return Date.today().add({year: 1});}
			}
		], 
	...
	// Remove all existing DateRangePicker containers
	$("#drpc_" + options.name + "").each(function(){
		$(this).detach();
	})
	// Remove all existing DateRangePicker arrows
	$("#drpa_" + options.name + "").each(function(){
		var arrowDiv = $(this);
		$(".DateRangePicker").each(function(){
			$(this).detach();
			arrowDiv.parent().append($(this));
		})
		arrowDiv.detach();
	})
	...
	jQuery.fn.clickActions = function(rp, rpPickers, doneBtn){
		if(jQuery(this).is('.ui-daterangepicker-specificDate')){
			...
			setTimeout(function(){doneBtn.fadeIn();});
		}
		else {
			//custom date range
				doneBtn.hide();
		...
		/***** MODIFIED BY GLENN ******/
		// This will make sure the chooser doesn't extend past the right side of the page
		var parent = $("#" + options.parent + "");
		var width = rp.width();
		var right = options.posX + width;
		if(right > parent.width() + parent.offset().left){
			var newPos = parent.width() + parent.offset().left - width;
			rp.parent().animate(
				{left: newPos}, 500,
				function() {
					width = rp.width();
	...
	//wrap and position
	rp.wrap('<div id="drpc_' + options.name + '" class="ui-daterangepickercontain"></div>');
	if(options.posX){
	...
	if(options.arrows && rangeInput.size()==1){
		var prevLink = jQuery('<a href="#" class="ui-daterangepicker-prev ui-corner-all" title="'+ options.prevLinkText +'"><span class="ui-icon ui-icon-circle-triangle-w"></span></a>');
		var nextLink = jQuery('<a href="#" class="ui-daterangepicker-next ui-corner-all" title="'+ options.nextLinkText +'"><span class="ui-icon ui-icon-circle-triangle-e"></span></a>');
		jQuery(this)
		.addClass('ui-rangepicker-input ui-widget-content')
		.wrap('<div id="drpa_' + options.name + '" class="ui-daterangepicker-arrows ui-widget ui-widget-header ui-helper-clearfix ui-corner-all"></div>')
		.before( prevLink )
		.before( nextLink )
		.parent().find('a').click(function(){
			var rangeStart = rpPickers.find('.range-start');
			var rangeEnd = rpPickers.find('.range-end');
			var dateA = rangeStart.datepicker('getDate');
			var dateB = rangeEnd.datepicker('getDate');
			var diff = dateB.getDayOfYear() - dateA.getDayOfYear();

			// These two checks fix bugs caused by different time zones (ie. Daylight savings time)
			if(dateA.getUTCOffset() > dateB.getUTCOffset())
				diff++;
			else if(dateA.getUTCOffset() < dateB.getUTCOffset())
				diff--;
			
			if(jQuery(this).is('.ui-daterangepicker-prev')){ 
				jQuery(rangeEnd).datepicker("setDate", dateA.addDays(-1));
				jQuery(rangeStart).datepicker("setDate", dateA.addDays(-diff)).find('.ui-datepicker-current-day').trigger('click');
			}
			else{
				jQuery(rangeStart).datepicker("setDate", dateB.addDays(1));
				jQuery(rangeEnd).datepicker("setDate", dateB.addDays(diff)).find('.ui-datepicker-current-day').trigger('click');
			}
